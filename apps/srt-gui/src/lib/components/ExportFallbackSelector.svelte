...
<div class="flex min-w-0 flex-wrap items-center justify-between gap-3 {className}">
  <div class="flex min-w-[15rem] flex-1 items-center gap-2 text-xs font-semibold text-gray-300">
  ...
  <div class="relative ml-auto grid shrink-0 grid-cols-2 rounded-lg border border-white/10 bg-black/40 p-1">
    ...
    <button type="button" onclick={() => onchange('apkg')} class="relative z-10 flex items-center gap-1.5 whitespace-nowrap rounded-md px-3 py-1.5 text-xs font-bold {value === 'apkg' ? 'text-emerald-200' : 'text-gray-400'}">
    ...
    <button type="button" onclick={() => onchange('tsv')} class="relative z-10 whitespace-nowrap rounded-md px-3 py-1.5 text-xs font-bold {value === 'tsv' ? 'text-violet-200' : 'text-gray-400'}">TSV</button>
  </div>
</div>
