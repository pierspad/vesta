<script lang="ts">
  ...
  let step = $state<"languages" | "preferences">("languages");
  ...
  let exportFormat = $state<"apkg" | "tsv">("apkg");
...
<div class="fixed inset-0 z-[100] flex items-center justify-center overflow-hidden bg-black/80 p-6" role="dialog" aria-modal="true">
  <div class="w-full max-w-5xl overflow-visible rounded-2xl border border-indigo-400/30 bg-gray-900 p-7 shadow-2xl">
    <div class="flex items-start justify-between gap-5">
      ...
      {#if mode}<span class="rounded-full border border-indigo-400/25 bg-indigo-500/10 px-3 py-1 text-xs text-indigo-200">{mode === "quick" ? (it ? "Rapido" : "Quick") : "Expert"} · {step === "languages" ? "1" : "2"}/{mode === "custom" ? "2" : "1"}</span>{/if}
    </div>

    {#if mode === null}
      <div class="mt-7 grid gap-4 sm:grid-cols-2">
        <button class="rounded-xl border border-emerald-400/30 bg-emerald-500/10 p-5 text-left hover:bg-emerald-500/20" onclick={() => { mode = "quick"; step = "languages"; }}><strong class="text-white">{it ? "Setup rapido" : "Quick setup"}</strong><p class="mt-2 text-xs text-gray-400">{it ? "Interfaccia semplice, APKG e impostazioni consigliate." : "Simple interface, APKG and recommended defaults."}</p></button>
        <button class="rounded-xl border border-indigo-400/30 bg-indigo-500/10 p-5 text-left hover:bg-indigo-500/20" onclick={() => { mode = "custom"; step = "languages"; }}><strong class="text-white">{it ? "Setup personalizzato" : "Custom setup"}</strong><p class="mt-2 text-xs text-gray-400">{it ? "Modalità Expert, formati media e trascrizione locale." : "Expert mode, media formats and local transcription."}</p></button>
      </div>
    {:else if step === "languages"}
      ...
      <div class="mt-7 flex justify-between"><button class="btn-secondary px-4 py-2" onclick={() => mode = null}>{it ? "Indietro" : "Back"}</button>{#if mode === "custom"}<button class="rounded-lg bg-indigo-500 px-5 py-2 font-semibold text-white" onclick={() => step = "preferences"}>{it ? "Continua" : "Continue"}</button>{:else}<button class="rounded-lg bg-indigo-500 px-5 py-2 font-semibold text-white" onclick={finish}>{it ? "Completa setup" : "Finish setup"}</button>{/if}</div>
    {:else}
      <div class="mt-7 grid gap-4 lg:grid-cols-[2fr_1fr]">
        <div class="rounded-xl border border-white/10 bg-white/[0.03] p-2"><div class="grid grid-cols-2 gap-1 rounded-lg bg-black/20 p-1"><button class="flex items-center justify-center gap-2 rounded-md px-4 py-2.5 text-sm font-semibold transition-colors {exportFormat === 'apkg' ? 'bg-emerald-500/20 text-emerald-200 ring-1 ring-emerald-400/40' : 'text-gray-400 hover:text-white'}" onclick={() => exportFormat = "apkg"}><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg> APKG <span class="text-[10px] opacity-70">{it ? "consigliato" : "recommended"}</span></button><button class="flex items-center justify-center gap-2 rounded-md px-4 py-2.5 text-sm font-semibold transition-colors {exportFormat === 'tsv' ? 'bg-sky-500/20 text-sky-200 ring-1 ring-sky-400/40' : 'text-gray-400 hover:text-white'}" onclick={() => exportFormat = "tsv"}><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg> TSV</button></div></div>
        <button class="rounded-xl border p-4 text-left {compactAudio ? 'border-cyan-400/50 bg-cyan-500/10' : 'border-white/10 bg-white/[0.03]'}" onclick={() => compactAudio = !compactAudio}><p class="font-semibold text-white">♫ {it ? "Audio compatto" : "Compact audio"}</p><p class="mt-1 text-xs text-gray-400">Opus · {compactAudio ? "ON" : "OFF"}</p></button>
      </div>
      ...
      <div class="mt-7 flex justify-between"><button class="btn-secondary px-4 py-2" disabled={installing} onclick={() => step = "languages"}>{it ? "Indietro" : "Back"}</button><button class="rounded-lg bg-indigo-500 px-5 py-2 font-semibold text-white disabled:opacity-50" disabled={installing} onclick={finish}>{installing ? (it ? "Installazione…" : "Installing…") : (it ? "Completa setup" : "Finish setup")}</button></div>
    {/if}
  </div>
</div>
